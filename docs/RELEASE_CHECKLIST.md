...
- `tools/run_operator_callable.ps1 -FunctionName seedStarterRegionData`로 부산 지역, POI, 펫 템플릿, 대사 시드를 반영한다.
- TourAPI 최신 장소 동기화가 필요하면 `tools/run_operator_callable.ps1 -FunctionName syncBusanPois`를 호출한다.
...
- 위치 권한을 허용하고, 부산 기준 위치 또는 실제 현장 위치로 지도와 POI 목록이 정상 표시되는지 확인한다.
...
- 도감 화면의 `부산 탐험 여권`에서 발견한 마실펫과 미발견 스탬프가 카테고리별로 구분되는지 확인한다.
