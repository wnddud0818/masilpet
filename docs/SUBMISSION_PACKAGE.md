...
2. 오늘의 산책 코스가 위치 확인, 첫 도장, 펫 교감, 알 부화 준비를 한 화면에서 안내합니다.
...
위치 권한을 바로 허용하기 어려운 심사 환경에서는 지도 화면의 `전국 기본 지도 보기`를 누르면 전국 기본 POI 기준으로 첫 체크인 루프를 확인할 수 있습니다. 이 경로는 기기 내 진행 모드에서만 체크인을 열고, Firebase 서버 모드에서는 실제 위치 확인을 계속 요구합니다.

## 시연 흐름

1. 온보딩에서 MasilPet의 지역 탐험 구조를 확인합니다.
2. 지도 탭에서 오늘의 산책 코스, 현재 위치, 주변 산책지와 POI 카테고리 필터를 확인합니다.
3. 위치 권한이 없으면 `전국 기본 지도 보기`, 실기기에서는 `현재 위치 확인하기`로 첫 도장 후보를 엽니다.
4. 150m 안의 장소에서 `여기에 도장 찍기`를 눌러 `방문 인증` 도장과 지도 탭의 `방문 인증 완료` 카드를 확인합니다.
5. 마실펫 탭에서 말풍선과 `쓰다듬기`, `오늘 해준 것`으로 장소 맥락 대사, `같이 다녀온 곳`, 능력치 변화를 확인합니다.
6. 하우스에서 `마당`의 펫·공·밥그릇 반응, 알 걸음 진행도, `오늘의 돌봄`, `다음 외출`을 확인합니다.
7. 도감에서 수집률, 지역·발견 필터, `미발견` 칸을 확인하고 `다음 발견 후보`의 `지도에서 탐험하기`가 지도 목표 카테고리로 이어지는지 확인합니다.
8. 하단 텍스트 탭 또는 좌측 사이드 레일의 네비게이션 배지로 체크인, 대화, 알, 도감, 연속 산책을 확인합니다.
9. 기록 탭에서 `WALKING PASSPORT` 도장 달력, 오늘의 리포트, 수첩 목표, 최근 산책, 개인정보 처리방침, 진행도 동기화 상태를 확인합니다.

## 제출 전 증빙

- `tools/release_preflight.ps1` 실행 결과
- `tools/release_evidence.ps1`의 Functions moderate npm audit 0 vulnerabilities 결과
...
- 기록 탭 `앱 정보와 연결`의 앱 버전, 빌드 채널, 빌드 시각
- 기록 탭의 오늘의 리포트와 `리포트 복사하기` 동선
- 기록 탭 리포트 태그의 성과 등급, 점수, 성장 루프, 카테고리 진척
- 하단·좌측 네비게이션의 체크인, 대화, 알, 도감, 연속 산책 진행 배지
- 기록 탭의 수첩 목표 진행률과 다음 행동 안내
- 기록 탭의 최근 산책 타임라인과 실제 적용된 체크인 보상 상세 표시
- 기록 탭의 데이터·지도 출처와 지도 타일 설정 표시
- 지도 탭의 방문 인증 도장 오버레이, 방문 인증 완료 카드, 보상 요약 표시
- 지도 탭의 오늘의 산책 코스, POI 카테고리 필터와 선택 카테고리 목록 표시
- 도감·하우스의 다음 목표 버튼이 지도 목표 카테고리와 POI 필터로 연결되는지 표시
- 지도 POI 카드의 장소 데이터 출처 표시
- 마실펫 탭의 말풍선, `같이 다녀온 곳`, 남은 쓰다듬기 횟수 표시
- 하우스 탭의 마당 상호작용, 알 걸음 진행도, 바로 부화 동선 표시
- 첫 로딩 화면과 JavaScript 비활성화 대체 안내 확인
- 배포 URL 링크 프리뷰의 제목, 설명, 대표 이미지 확인
- PWA 설치 가능 여부, 지도/개인정보 바로가기, wide/narrow manifest 스크린샷 확인
- `/privacy.html` 접속 확인
- 기록 탭 `진행도 관리`의 진행도 초기화 확인
- 온보딩 wide/narrow, 지도, 마실펫, 기록 화면 스크린샷
