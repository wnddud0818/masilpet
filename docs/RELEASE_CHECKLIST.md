...
- 위치 권한을 바로 허용하기 어려운 심사 환경에서는 `기본 위치로 체험`을 눌러 첫 체크인 후보, 0m 거리, `지금 체크인 가능` 신호가 표시되는지 확인한다.
- 지도 화면의 `카테고리 필터`가 전체 POI와 선택한 카테고리 POI 수를 구분하고 목록을 즉시 좁히는지 확인한다.
- 도감과 하우스의 다음 목표 버튼을 누른 뒤 지도 화면에 `지도 목표`가 표시되고 해당 카테고리 필터가 선택되는지 확인한다.
- 지도 화면의 `오늘의 산책 루트`와 `추천 코스`가 위치 확인, 첫 체크인, 마실펫 교감, 알 부화 준비 상태와 상위 3개 후보를 겹침 없이 표시하는지 확인한다.
- 현재 위치 확인 전 또는 위치 확인 15분 경과 후에는 체크인 버튼이 잠기는지 확인한다.
- 150m 밖에서는 체크인이 막히고, 150m 안에서는 체크인 보상이 적용되는지 확인한다.
- 체크인 직후 지도 화면의 `방문 인증 완료` 카드가 장소, 카테고리, 거리, 보상, `마실펫에게 들려주기`, `리포트 보기` 동선을 표시하는지 확인한다.
- 하단 또는 좌측 네비게이션 배지가 체크인 수, 남은 대화, 알, 미발견 도감, 탐험 준비 상태를 표시하는지 확인한다.
...
- 체크인 후 마실펫 화면의 `동행 대화` 카드가 방문 카테고리 맥락 대사, 최근 방문 보상, 대화 가능 횟수를 표시하고 `마실펫과 대화하기` 동선이 동작하는지 확인한다.
...
- 내 정보 화면의 `오늘의 탐험 리포트`가 방문 수, 최근 장소, 보상, 다음 추천지를 표시하고 `요약 복사`가 동작하는지 확인한다.
- 내 정보 화면의 `오늘의 성과 카드`가 성과 등급, 점수, 성장 루프, 도감 진척, 다음 액션, 카테고리 기억을 표시하는지 확인한다.
- 내 정보 화면의 `탐험 배지`가 위치 인증, 첫 발자국, 장소 이야기꾼, 연속 탐험, 부화 준비 진행률과 다음 행동을 표시하는지 확인한다.
