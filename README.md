...
마실펫은 지역을 걸으며 나만의 반려펫을 만나고 키우는 위치 기반 육성 앱입니다. 첫 탐험 지역은 부산이며, 현재 저장소는 Flutter SDK가 없는 환경에서 직접 스캐폴딩한 소스 구조입니다. SDK 설치 후 플랫폼 템플릿을 붙여 실행할 수 있습니다.

## MVP 범위

- Firebase 익명 인증 기반 첫 실행
- 부산 첫 탐험 지역과 오리지널 마실펫 5종
...
앱 실행 후 `내 정보 > 서버 시드 준비`를 누르면 마실펫의 첫 지역 데이터와 사용자 초기화 함수가 호출됩니다.
