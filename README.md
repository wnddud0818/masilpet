...
- OpenStreetMap 타일 기반 부산 POI 지도와 카테고리 마커
...
- 지역 방문 맥락에 맞춘 마실펫 대사와 교감 액션
- 오늘의 하우스 플랜, 부산 탐험 여권, 도감, 탐험 준비 상태를 보여주는 진행률 화면
...
부산 지역, POI, 펫 템플릿, 대사 시드는 운영자 custom claim(`operator: true`)이 있는 계정으로만 callable Functions에서 반영할 수 있습니다. 일반 익명 사용자는 공용 데이터 쓰기와 TourAPI 동기화를 실행할 수 없습니다.
