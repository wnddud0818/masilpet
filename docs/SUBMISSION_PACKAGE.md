...
2. 지도 탭에서 오늘의 산책 루트, 추천 코스, 현재 위치, 주변 POI와 카테고리 필터를 확인합니다.
3. 위치 권한이 없으면 `기본 위치로 체험`, 실기기에서는 `현재 위치 확인`으로 첫 체크인 후보를 엽니다.
4. 150m 안의 장소에 체크인해 지도 탭의 `방문 인증 완료` 카드에서 펫 성장치, 알 진행도, 다음 행동을 확인합니다.
5. 마실펫 탭에서 `동행 대화` 카드와 `마실펫과 대화하기` 흐름으로 장소 맥락 대사, 최근 방문 보상, 능력치 변화를 확인합니다.
6. 하우스에서 오늘의 하우스 플랜, 집중 부화 알, 다음 외출지를 확인합니다.
7. 도감에서 성장 상태, 전국 탐험 여권, 보유 펫, 수집률을 확인하고 다음 발견 후보의 `지도에서 음식 장소 찾기`가 지도 목표 카테고리로 이어지는지 확인합니다.
8. 하단 또는 좌측 네비게이션 배지로 체크인, 대화, 알, 도감, 탐험 준비 상태를 확인합니다.
9. 내 정보 탭에서 오늘의 탐험 리포트, 오늘의 성과 카드, 연속 탐험 배지, 최근 방문 기록, 개인정보 처리방침, 진행도 동기화 상태를 확인합니다.
...
- 내 정보 탭의 오늘의 성과 카드, 성과 등급, 성장 루프, 도감 목표 다음 행동
- 하단·좌측 네비게이션의 체크인, 대화, 알, 도감, 탐험 준비 진행 배지
- 내 정보 탭의 연속 탐험 배지와 다음 행동 안내
- 내 정보 탭의 최근 방문 기록과 실제 적용된 체크인 보상 상세 표시
- 내 정보 탭의 데이터·지도 출처와 지도 타일 설정 표시
- 지도 탭의 방문 인증 완료 카드, 보상 요약, 마실펫·리포트 다음 행동 표시
- 지도 탭의 추천 코스, POI 카테고리 필터와 선택 카테고리 목록 표시
- 도감·하우스의 다음 목표 버튼이 지도 목표 카테고리와 POI 필터로 연결되는지 표시
- 지도 POI 카드의 장소 데이터 출처 표시
- 마실펫 탭의 동행 대화 카드, 최근 방문 보상, 대화 가능 횟수 표시
