...
void main() {
  ...
  test('manual step test controls are not exposed in production UI', () {
    ...
    expect(profileSource, contains('기본 위치로 체험')); // 위치 안내 문구
  ...
  test('submission package is ready for contest handoff', () {
    ...
    expect(readme, contains('산책 코스'));
    expect(readme, contains('방문 인증 완료'));
    expect(readme, contains('오늘의 리포트'));
    expect(readme, contains('방문 인증 도장'));
    expect(readme, contains('성장 루프'));
    expect(readme, contains('텍스트 탭 배지'));
    expect(readme, contains('연속 산책'));
    expect(readme, contains('수첩 목표'));
    expect(readme, contains('종이 수첩'));
    ...
    expect(submission, contains('전국 기본 지도 보기'));
    expect(submission, contains('쓰다듬기'));
    expect(submission, contains('같이 다녀온 곳'));
    expect(submission, contains('산책 코스'));
    expect(submission, contains('카테고리 필터'));
    expect(submission, contains('지도 목표 카테고리'));
    expect(submission, contains('방문 인증 완료'));
    expect(submission, contains('리포트 복사하기'));
    expect(submission, contains('오늘의 리포트'));
    expect(submission, contains('WALKING PASSPORT'));
    expect(submission, contains('성과 등급'));
    expect(submission, contains('지도 목표 카테고리'));
    expect(submission, contains('텍스트 탭'));
    expect(submission, contains('연속 산책 진행 배지'));
    expect(submission, contains('연속 산책'));
    expect(submission, contains('수첩 목표'));
    expect(submission, contains('마당 상호작용'));
    ...
    expect(checklist, contains('전국 기본 지도 보기'));
    expect(checklist, contains('moderate npm audit'));
    expect(checklist, contains('여기에 도장 찍기'));
    expect(checklist, contains('오늘의 산책 코스'));
    expect(checklist, contains('분류 필터'));
    expect(checklist, contains('분류 필터가 선택'));
    expect(checklist, contains('방문 인증 완료'));
    expect(checklist, contains('리포트 복사하기'));
    expect(checklist, contains('쓰다듬기'));
    expect(checklist, contains('같이 다녀온 곳'));
    expect(checklist, contains('오늘의 리포트'));
    expect(checklist, contains('WALKING PASSPORT'));
    expect(checklist, contains('성장 루프'));
    expect(checklist, contains('카테고리 진척'));
    expect(checklist, contains('사이드 레일'));
    expect(checklist, contains('미발견 도감'));
    expect(checklist, contains('연속 산책'));
    expect(checklist, contains('수첩 목표'));
    expect(checklist, contains('마당'));
